...
import app.lifewin.interfaces.DragNDropListeners;
 ...
 * 
 */
public class TaskDragNDropListView extends ExpandableListView {
	...
	private int prevY = -1;
	private int backgroundColor = 0xe077A89D; // different color to identify
	private int defaultBackgroundColor=0xdf77A89D;
	private float screenHeight;
	private float dragRatio;
	private ImageView mDragView;
	private TaskDragNDropAdapter adapter;
	...
	public TaskDragNDropListView(Context context, AttributeSet attrs) {
		super(context, attrs);
...
//					pressedItem = false;
		...
		Log.e("Math.abs(prevY - y)",Math.abs(prevY - y)+"");
...
//				pressedItem = true;
		...
		case MotionEvent.ACTION_MOVE:
			...
			Logger.error("speed   :-->"+ speed+"   Y  :-> "+y);
			if (getLastVisiblePosition() < getCount() && speed > 0) {
				smoothScrollBy(speed, 1);
			}
			Logger.error("Position   :-->"+ getFirstVisiblePosition());
	...
	public void setAdapter(ExpandableListAdapter adapter) {
		...
		this.adapter = (TaskDragNDropAdapter) adapter;
	}
